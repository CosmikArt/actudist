...
Continuous log-likelihood with right-censoring and one- or two-sided
truncation for severity. Discrete log-likelihood for frequency. Both
drivers minimize the negative log-likelihood in an unconstrained
reparameterization (see :mod:`actudist._numerics`) using L-BFGS-B with a
Nelder-Mead fallback. ``profile_likelihood_ci`` builds a
:math:`\\chi^{2}_{1}`-based CI by walking outward from the MLE.

Distribution classes declare ``_transforms()`` and ``_initial_guess()``;
they should not call ``scipy.optimize`` directly.
"""
...
def loglik_continuous(
    ...
    *,
    ...
    r"""Log-likelihood for a ``SeverityDistribution``.

    Each ``data[i]`` is exact unless ``censored[i]`` is True. Observations
    are conditioned on :math:`(\ell, u]` when truncation bounds are given.
...
def fit_continuous_mle(
    ...
    *,
    ...
    """Fit ``dist_class`` by MLE in unconstrained space. L-BFGS-B with a
    Nelder-Mead fallback when the gradient method fails."""
...
def profile_likelihood_ci(
    ...
    *,
    ...
    def _walk(direction: int) -> float | None:
        # direction is -1 or +1; return crossing or None if not found
        bracket_inner = point
        # initial step size
...
def fit_discrete_mle(
    ...
    *,
    ...
    """L-BFGS-B MLE for a frequency distribution. No censoring or
    truncation hooks."""
    transforms = dist_class._transforms()
    if initial_params is None:
        initial_params = dist_class._initial_guess(data)
