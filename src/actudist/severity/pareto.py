"""Pareto Type II (Lomax) severity. Survival function
:math:`S(x) = (\\theta/(\\theta+x))^{\\alpha}`."""
...
class Pareto(SeverityDistribution):
    ...
    Lomax / Pareto Type II in Klugman §A.2.3.4.
    """
    ...
    def pdf(self, x: ArrayLike) -> np.ndarray:
        ...
        xx = x[m]
        # log f(x) = log(a) + a*log(t) - (a+1)*log(x+t)
        log_pdf = np.log(a) + a * np.log(t) - (a + 1.0) * np.log(xx + t)
